...
app.post('/api/post', restrict, (req,res)=>{
  try {
...
app.delete('/api/post/:id',restrict,(req,res)=>{
    try {
...
app.put('/api/post/:id',restrict,(req,res)=>{
  try{
    const post = db.prepare("SELECT id FROM Posts WHERE id = ?").get(req.params.id);
    if (!post) {
      console.log("No Post with that ID!!");
...
app.post('/api/tag', restrict, (req,res)=>{
  try {
...
app.delete('/api/tag/:id',restrict,(req,res)=>{
    try {
...
app.put('/api/tag/:id',restrict,(req,res)=>{
  try{
    const tag = db.prepare("SELECT id FROM Tags WHERE id = ?").get(req.params.id);
    if (!tag) {
      console.log("No Tag with that ID!!");
